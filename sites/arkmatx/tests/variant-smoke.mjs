...
for(const variant of variants){
 for(const profile of profiles){
  ...
  try{
   ...
   let state=await page.evaluate(()=>({dataset:document.body.dataset.variant,stored:localStorage.getItem('arkmatx-visual-variant'),location:document.querySelector('#location')?.textContent||''}));
   if(state.dataset!==variant||state.stored!==variant)failures.push(`[${label}] forced variant failed: ${JSON.stringify(state)}`);
   await page.goto(baseURL,{waitUntil:'networkidle',timeout:30000});
   await page.waitForTimeout(350);
   state=await page.evaluate(()=>({dataset:document.body.dataset.variant,stored:localStorage.getItem('arkmatx-visual-variant')}));
   if(state.dataset!==variant||state.stored!==variant)failures.push(`[${label}] variant did not persist: ${JSON.stringify(state)}`);
