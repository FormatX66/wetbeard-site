...
<style>
:root{font-family:ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color-scheme:dark;background:#111;color:#eee}body{margin:0;background:#111}main{max-width:1180px;margin:auto;padding:20px}.card{background:#1c1c1c;border:1px solid #333;border-radius:14px;padding:16px;margin:0 0 16px}h1,h2,h3{margin-top:0}input,select,textarea,button{font:inherit;background:#0f0f0f;color:#eee;border:1px solid #444;border-radius:8px;padding:9px}input,select,textarea{width:100%;box-sizing:border-box}input[type=checkbox]{width:auto}button{cursor:pointer;background:#2a2a2a}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:12px}.row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.muted{color:#aaa}.error{color:#ff9d9d}.good{color:#9dffb4}.badge{padding:3px 8px;border-radius:999px;background:#333;font-size:.8rem}.risk-high{font-weight:700}.tablewrap{overflow:auto;max-height:58vh}table{width:100%;border-collapse:collapse;font-size:.9rem}th,td{text-align:left;vertical-align:top;padding:8px;border-bottom:1px solid #333}th{position:sticky;top:0;background:#1c1c1c}code{word-break:break-all}.login{max-width:420px;margin:12vh auto}.secret{background:#0b0b0b;padding:8px;border-radius:8px;display:block;overflow:auto}.split{display:grid;grid-template-columns:1fr 1fr;gap:16px}.small{font-size:.86rem}.bot-score{font-size:1.05rem;font-weight:800}.bot-low{color:#9dffb4}.bot-mid{color:#ffe39d}.bot-high{color:#ffb19d}.bot-very-high{color:#ff8c8c}.modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.78);z-index:1000;padding:18px;overflow:auto}.modal.open{display:block}.modal-panel{max-width:1050px;margin:2vh auto;background:#171717;border:1px solid #444;border-radius:14px;padding:18px;box-shadow:0 16px 60px rgba(0,0,0,.55)}.metric-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px}.metric{background:#0e0e0e;border:1px solid #333;border-radius:10px;padding:10px}.metric strong{display:block;font-size:1.15rem}.signal{border-left:3px solid #555;padding:8px 10px;margin:8px 0;background:#111}.report-comments{max-height:360px;overflow:auto;border:1px solid #333;border-radius:10px}.report-comment{padding:10px;border-bottom:1px solid #333}.report-comment:last-child{border-bottom:0}a{color:inherit}@media(max-width:760px){.split{grid-template-columns:1fr}}
...
<div class="card">
  <div class="row" style="justify-content:space-between"><div><h2 style="margin:0">Bot / automation analysis</h2><div class="muted small">Behavioral estimate only — not proof that an account is automated.</div></div><div class="row"><button onclick="loadBots()">Refresh</button><a href="api.php?action=bot_export">Export bot report CSV</a></div></div>
  <div class="tablewrap"><table><thead><tr><th>Bot %</th><th>Account</th><th>Platform</th><th>Confidence</th><th>Comments</th><th>Top signals</th><th></th></tr></thead><tbody id="bots"></tbody></table></div>
</div>
...
<div class="modal" id="botModal" onclick="if(event.target===this)closeBotReport()">
  <div class="modal-panel">
    <div class="row" style="justify-content:space-between"><h2 style="margin:0">Automation Analysis Report</h2><button onclick="closeBotReport()">Close</button></div>
    <div id="botReportContent"><p class="muted">Loading…</p></div>
  </div>
</div>
...
function botClass(v){v=Number(v)||0;return v>=75?'bot-very-high':v>=50?'bot-high':v>=25?'bot-mid':'bot-low';}
...
async function loadBots(){try{const j=await api('bot_users');bots.innerHTML=(j.accounts||[]).map(a=>`<tr><td><span class="bot-score ${botClass(a.bot_percentage)}">${Number(a.bot_percentage)||0}%</span><br><small>${esc(a.label)}</small></td><td><strong>${esc(a.username||'(unknown)')}</strong><br><small>${esc(a.user_id)}</small></td><td>${esc(a.platform)}</td><td>${esc(a.confidence)}</td><td>${Number(a.comment_count)||0}</td><td>${(a.top_signals||[]).map(s=>`<span class="badge">${esc(s.name)} +${esc(s.points)}</span>`).join(' ')||'<span class="muted">No strong automation signals</span>'}</td><td><button onclick="openBotReport('${esc(a.report_id)}')">Full report</button></td></tr>`).join('')||'<tr><td colspan="7" class="muted">No account data yet.</td></tr>';}catch(e){bots.innerHTML='<tr><td colspan="7" class="error">'+esc(e.message)+'</td></tr>';}}

async function openBotReport(id){botModal.classList.add('open');botReportContent.innerHTML='<p class="muted">Loading report…</p>';try{const r=await fetch('api.php?action=bot_report&id='+encodeURIComponent(id),{headers:{'Accept':'application/json'}});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'Report failed');renderBotReport(j.report);}catch(e){botReportContent.innerHTML='<p class="error">'+esc(e.message)+'</p>';}}
function closeBotReport(){botModal.classList.remove('open');}
function renderBotReport(r){const m=r.metrics||{};const signals=(r.signals||[]).length?(r.signals||[]).map(s=>`<div class="signal"><strong>${esc(s.name)}: +${esc(s.points)} points</strong><div>${esc(s.value)}</div><div class="muted small">${esc(s.explanation)}</div></div>`).join(''):'<p class="muted">No strong automation signals detected.</p>';const common=(r.common_texts||[]).length?(r.common_texts||[]).map(x=>`<tr><td>${esc(x.text)}</td><td>${Number(x.count)||0}</td><td>${Number(x.accounts_using_text)||0}</td></tr>`).join(''):'<tr><td colspan="3" class="muted">No repeated text.</td></tr>';const sample=(r.comments||[]).map(c=>`<div class="report-comment"><div class="row"><span class="badge">${esc(c.risk_level)}</span><span class="muted small">${esc(c.created_time)}</span><span class="muted small">${esc(c.source_type)} ${esc(c.source_file)}</span></div><div style="margin-top:6px">${esc(c.text)}</div></div>`).join('')||'<div class="report-comment muted">No comment samples available.</div>';botReportContent.innerHTML=`<div style="margin:16px 0"><div class="bot-score ${botClass(r.bot_percentage)}" style="font-size:2.2rem">${Number(r.bot_percentage)||0}% bot likelihood</div><h3 style="margin:4px 0">${esc(r.username||'(unknown)')} · ${esc(r.platform)}</h3><div>${esc(r.label)} · <strong>${esc(r.confidence)} confidence</strong> · ${Number(r.comment_count)||0} comments</div><p class="muted small">${esc(r.methodology_note)}</p></div><h3>Key metrics</h3><div class="metric-grid"><div class="metric"><span class="muted">Duplicate comments</span><strong>${pct(m.duplicate_ratio)}</strong></div><div class="metric"><span class="muted">Shared across accounts</span><strong>${pct(m.cross_account_shared_ratio)}</strong></div><div class="metric"><span class="muted">≤60 sec bursts</span><strong>${pct(m.burst_60s_ratio)}</strong></div><div class="metric"><span class="muted">≤10 sec bursts</span><strong>${pct(m.burst_10s_ratio)}</strong></div><div class="metric"><span class="muted">Median interval</span><strong>${m.median_interval_seconds==null?'n/a':esc(m.median_interval_seconds)+'s'}</strong></div><div class="metric"><span class="muted">Minimum interval</span><strong>${m.minimum_interval_seconds==null?'n/a':esc(m.minimum_interval_seconds)+'s'}</strong></div><div class="metric"><span class="muted">Link comments</span><strong>${pct(m.link_ratio)}</strong></div><div class="metric"><span class="muted">Timing coverage</span><strong>${pct(m.timestamp_coverage)}</strong></div><div class="metric"><span class="muted">High-risk comments</span><strong>${Number(r.high_risk_count)||0}</strong></div><div class="metric"><span class="muted">Medium-risk comments</span><strong>${Number(r.medium_risk_count)||0}</strong></div></div><h3 style="margin-top:18px">Why this score</h3>${signals}<h3 style="margin-top:18px">Most repeated / shared text</h3><div class="tablewrap" style="max-height:260px"><table><thead><tr><th>Normalized text</th><th>This account</th><th>Accounts using it</th></tr></thead><tbody>${common}</tbody></table></div><h3 style="margin-top:18px">Comment evidence</h3><div class="report-comments">${sample}</div>`;}

document.addEventListener('keydown',e=>{if(e.key==='Escape')closeBotReport();});
loadStatus().then(()=>Promise.all([loadComments(),loadUsers(),loadBots()]));
</script>
<?php endif; ?>
</main></body></html>
