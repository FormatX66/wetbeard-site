...
port="${DEPLOY_PORT:-22}"; release="$RELEASE_ROOT/$RELEASE_ID"; next="${TARGET_PATH}.next"; previous="${TARGET_PATH}.previous"; target_parent="$(dirname "$TARGET_PATH")"
...
"${sshcmd[@]}" "set -e; mkdir -p '$target_parent' '$RELEASE_ROOT' '$BACKUP_ROOT'; if [ -d '$TARGET_PATH' ]; then tar -C \"\$(dirname '$TARGET_PATH')\" -czf '$BACKUP_ROOT/pre-$RELEASE_ID.tar.gz' \"\$(basename '$TARGET_PATH')\"; fi; rm -rf '$release'; mkdir -p '$release'"
